...
(
  ...
  printf '%s' "$ts_output" | jq -e '.harness.checks | map(.command) | index("npm run typecheck") != null' >/dev/null
  printf '%s' "$ts_output" | jq -e '.harness.checks | map(.command) | index("npm test") != null' >/dev/null
